...
public class NSGAIIStudy2 {
	...
	public static void main(String[] args) throws IOException {
		...
		// }
		String experimentBaseDirectory = "NSGAIIStudy2";
		String referenceFrontDirectory = "NSGAIIStudy2Fronts";
		...
		Experiment<DoubleSolution, List<DoubleSolution>> experiment = new ExperimentBuilder<DoubleSolution, List<DoubleSolution>>(
				"NSGAIIStudy2").setAlgorithmList(algorithmList).setProblemList(problemList)
	 ...
	 * {@link Problem} which form part of a {@link TaggedAlgorithm}, which is a
	 * decorator for class {@link Algorithm}. The {@link TaggedAlgorithm} has an
	 * optional tag component, that can be set as it is shown in this example,
	...
	static List<TaggedAlgorithm<List<DoubleSolution>>> configureAlgorithmList(List<Problem<DoubleSolution>> problemList,
		...
		for (int run = 0; run < independentRuns; run++) {
			...
			// NSGAIIBuilder<>(problemList.get(i), new SBXCrossover(0.9, 20.0),
			// new PolynomialMutation(1.0 /
			// problemList.get(i).getNumberOfVariables(), 20.0))
			// .setMaxEvaluations(25000)
			// .setPopulationSize(100)
			// .build();
			// algorithms.add(new
			// TaggedAlgorithm<List<DoubleSolution>>(algorithm, "NSGAIIb",
			// problemList.get(i), run));
			// }
			//
			// for (int i = 0; i < problemList.size(); i++) {
			// Algorithm<List<DoubleSolution>> algorithm = new
			// NSGAIIBuilder<>(problemList.get(i), new SBXCrossover(0.8, 50.0),
			// new PolynomialMutation(1.0 /
			// problemList.get(i).getNumberOfVariables(), 20.0))
			// .setMaxEvaluations(25000)
			// .setPopulationSize(100)
			// .build();
			// algorithms.add(new
			// TaggedAlgorithm<List<DoubleSolution>>(algorithm, "NSGAIIc",
			// problemList.get(i), run));
			// }
			//
			// for (int i = 0; i < problemList.size(); i++) {
			// Algorithm<List<DoubleSolution>> algorithm = new
			// NSGAIIBuilder<>(problemList.get(i), new SBXCrossover(0.7, 50.0),
			// new PolynomialMutation(1.0 /
			// problemList.get(i).getNumberOfVariables(), 20.0))
			// .setMaxEvaluations(25000)
			// .setPopulationSize(100)
			// .build();
			// algorithms.add(new
			// TaggedAlgorithm<List<DoubleSolution>>(algorithm, "NSGAIId",
			// problemList.get(i), run));
			// }
		}
		return algorithms;
